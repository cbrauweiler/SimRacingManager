...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($row && mfaVerify($row['mfa_secret'], $code)) {
        ...
        $_SESSION['admin_id']   = $_SESSION['mfa_pending_id'];
    ...
    $error = 'Ungültiger Code. Bitte prüfen und erneut versuchen.';
}
...
<title>Zwei-Faktor-Authentifizierung – <?= h($s['league_name']??'') ?></title>
...
.sub{color:#8888a0;font-size:.85rem;margin-bottom:28px}
.code-input{width:100%;background:#1c1c28;border:1px solid #2a2a3a;border-radius:4px;padding:14px;color:#f0f0f5;font-size:2rem;font-family:'Barlow Condensed',sans-serif;font-weight:700;letter-spacing:.3em;text-align:center;outline:none;transition:border-color .2s;margin-bottom:18px}
...
.hint{color:#8888a0;font-size:.78rem;margin-top:14px;line-height:1.5}
</style>
</head><body>
<div class="box">
  <div class="icon">🔐</div>
  <div class="title">Zwei-Faktor <span style="color:var(--primary)">Code</span></div>
  <div class="sub">Öffne deine Authenticator-App und gib den 6-stelligen Code ein</div>
  ...
  <div class="hint">Code ist 30 Sekunden gültig.<br/>Account: <strong><?= h($_SESSION['mfa_pending_user'] ?? '') ?></strong></div>
  <a href="<?= SITE_URL ?>/admin/login.php" class="back" onclick="<?php
    session_destroy(); ?>">← Zurück zum Login</a>
</div>
<script>
