...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_entries') {
        ...
        foreach ($entries as $entryId => $e) {
            ...
            $db->prepare("
                ...
                SET position=?, laps=?, total_time=?, gap=?,
        ...
        // Nur ein FL erlaubt – wenn mehrere gecheckt wurden, nur den mit der besten Position behalten
        $db->prepare("
            UPDATE result_entries SET is_fastest_lap=0
            WHERE result_id=? AND is_fastest_lap=1
              AND id NOT IN (
                SELECT id FROM (
                    SELECT id FROM result_entries
                    WHERE result_id=? AND is_fastest_lap=1
                    ORDER BY position ASC LIMIT 1
                ) t
              )
        ")->execute([$resultId, $resultId]);
    ...
    if ($action === 'delete_entry') {
        ...
        if ($entryId) {
            $db->prepare("DELETE FROM result_entries WHERE id=? AND result_id=?")
               ->execute([$entryId, $resultId]);
        }
    ...
    if ($action === 'recalc_points') {
        ...
        $entries = $db->prepare("SELECT id,position FROM result_entries WHERE result_id=? AND dnf=0 AND dsq=0 ORDER BY position ASC");
        $entries->execute([$resultId]);
...
$seasonDrivers->execute([$result['season_id']]);
$seasonDrivers = $seasonDrivers->fetchAll();

$seasonTeams = $db->prepare("SELECT id,name FROM teams WHERE season_id=? ORDER BY name");
$seasonTeams->execute([$result['season_id']]);
$seasonTeams = $seasonTeams->fetchAll();
...
<div class="flex gap-2 mb-3" style="flex-wrap:wrap">
  ...
  <form method="post" style="display:inline" onsubmit="return confirm('Punkte neu berechnen? Aktuelle Punkte werden überschrieben!')">
    ...
    <button class="btn btn-secondary">🔄 Punkte neu berechnen</button>
  </form>
</div>

<!-- ============================================================
     Hauptformular – NUR Speichern, kein Delete-Button drin!
     Delete läuft über separates Formular außerhalb.
     ============================================================ -->
    ...
    <table class="admin-table" style="font-size:.85rem">
      <thead>
        <tr>
          ...
          <th style="width:92px">Bestzeit</th>
      ...
      <tbody>
        ...
        <tr id="row-<?= $eid ?>" style="opacity:<?= ($e['dnf']||$e['dsq']) ? '.55' : '1' ?>">
          ...
          <td>
            <!-- Hidden damit der Wert auch bei disabled übermittelt wird -->
            <input type="hidden" name="entry[<?= $eid ?>][position]" id="pos-hidden-<?= $eid ?>" value="<?= (int)$e['position'] ?>"/>
            <input type="number"
                   value="<?= (int)$e['position'] ?>"
                   min="1" max="99"
                   class="form-control" style="width:50px;padding:4px 6px;text-align:center"
                   <?= ($e['dnf']||$e['dsq']) ? 'disabled' : '' ?>
                   oninput="document.getElementById('pos-hidden-<?= $eid ?>').value=this.value"/>
          </td>
          ...
          <td>
            <?php if($e['color']): ?>
              <span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:<?= h($e['color']) ?>;margin-right:4px;vertical-align:middle"></span>
            <?php endif; ?>
          ...
          <td style="text-align:center">
            <!-- Checkbox: immer 0 senden, bei checked auf 1 -->
            <input type="hidden" name="entry[<?= $eid ?>][is_fastest_lap]" value="0"/>
            <input type="checkbox" name="entry[<?= $eid ?>][is_fastest_lap]"
                   value="1"
                   <?= $e['is_fastest_lap'] ? 'checked' : '' ?>
                   onchange="handleFL(this, <?= $eid ?>)"/>
          </td>

          <td style="text-align:center">
            <input type="hidden" name="entry[<?= $eid ?>][dnf]" value="0"/>
            <input type="checkbox" name="entry[<?= $eid ?>][dnf]"
                   value="1"
                   <?= $e['dnf'] ? 'checked' : '' ?>
                   onchange="handleStatus(this, <?= $eid ?>)"/>
          </td>

          <td style="text-align:center">
            <input type="hidden" name="entry[<?= $eid ?>][dsq]" value="0"/>
            <input type="checkbox" name="entry[<?= $eid ?>][dsq]"
                   value="1"
                   <?= $e['dsq'] ? 'checked' : '' ?>
                   onchange="handleStatus(this, <?= $eid ?>)"/>
          </td>
          ...
          <td>
            <input type="number" name="entry[<?= $eid ?>][points]"
                   ...
                   class="form-control pts-input-<?= $eid ?>"
                   style="width:64px;padding:4px 6px;text-align:center;font-weight:700;color:var(--primary)"/>
          </td>

          <td>
            <!-- Delete: eigenes Formular AUSSERHALB des Haupt-Forms via JS -->
            <button type="button"
                    class="btn btn-danger btn-sm btn-icon"
                    onclick="deleteEntry(<?= $eid ?>, '<?= h(addslashes($e['driver_name'] ?? $e['driver_name_raw'] ?? '?')) ?>')"
...
<!-- Delete-Formular – außerhalb des Haupt-Forms, via JS befüllt -->
...
<div class="notice notice-info" style="font-size:.82rem">
  💡 <strong>Punkte neu berechnen</strong> überschreibt alle aktuellen Punkte anhand der konfigurierten Punktetabelle.
  Punkteabzüge durch Strafen werden separat im Strafensystem verwaltet und automatisch in der Wertung berücksichtigt.
</div>

<script>
// FL: nur einer darf angehakt sein
function handleFL(cb, id) {
    if (cb.checked) {
        document.querySelectorAll('input[type=checkbox][name*="[is_fastest_lap]"]').forEach(function(el) {
            if (el !== cb && el.value === '1') el.checked = false;
        });
    }
}

// DNF/DSQ: Position deaktivieren, Punkte auf 0 setzen, Zeile abdunkeln
function handleStatus(cb, id) {
    var row    = document.getElementById('row-' + id);
    var dnfCb  = row.querySelector('input[name*="[dnf]"][value="1"]');
    var dsqCb  = row.querySelector('input[name*="[dsq]"][value="1"]');
    var isDead = (dnfCb && dnfCb.checked) || (dsqCb && dsqCb.checked);

    row.style.opacity = isDead ? '0.55' : '1';

    // Position: hidden input aktualisieren
    var posHidden = document.getElementById('pos-hidden-' + id);
    var posVisible = row.querySelector('input[type=number][oninput]');
    if (posVisible) posVisible.disabled = isDead;
    if (isDead && posHidden) posHidden.value = '';

    // Punkte auf 0 bei DNF/DSQ
    var ptsInput = row.querySelector('.pts-input-' + id);
    if (isDead && ptsInput) ptsInput.value = 0;
}

// Delete: separates Formular außerhalb des Haupt-Forms
